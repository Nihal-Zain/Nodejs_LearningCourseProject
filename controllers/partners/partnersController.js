// controllers/partners/partnersController.js
...
exports.getAllPartners = async (req, res) => {
  try {
    const [results] = await executeQuery("SELECT * FROM partners ORDER BY id DESC");
    res.json(results);
  } catch (err) {
...
exports.updatePartner = async (req, res) => {
  try {
    ...
    if (logo) {
      updateQuery = "UPDATE partners SET name = ?, logo = ?, link = ? WHERE id = ?";
      updateParams = [name, logo, link, id];
    } else {
      updateQuery = "UPDATE partners SET name = ?, link = ? WHERE id = ?";
...
exports.deletePartner = async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await executeQuery("DELETE FROM partners WHERE id = ?", [id]);
